...
namespace {
    std::unordered_map<std::string, const std::string> initialize_entities() {
        return {
...
namespace MAL {
    ...
    void TextUtility::parse_html_entities(std::string& str)
    {
        ...
        for (; pos != std::string::npos; pos = str.find("&", pos + 1)) {
            ...
            auto iter = html_entities.find(entity);
            if (G_LIKELY(iter != std::end(html_entities))) {
                ...
                try {
                    ...
                    html_entities.emplace(entity, utf8);
